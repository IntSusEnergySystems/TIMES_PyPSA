...
- **Adequacy**: parent–child heat identities hold, no disallowed double-count overlaps, and topology is clean for the reference scenario. ~76% of the DMD coverage gap is consumption-side (n+1/n+2) of already-exported production; ~20% is material accounting **or commodities missing from the mapping**; ~5% residual. The 2026-07-25 audit closed every defect that would change the demand CSVs; what remains under [Open points](#open-points) are coupling decisions.
...
Known-zero allowlist (`expect=zero`): `ammonia`, `methanol`, `total international navigation` — structurally absent from TIMES-WAL. `coal` used to be on this list; after the `INDCOA00`/`INDCOK00` label unswap it is 6.80 PJ in 2030 and non-zero in every year.
...
**No open defect is left from the 2026-07-25 audit.** Two questions stay open as
*coupling decisions* rather than bugs, both documented in
[aggregation.md § Audit 2026-07-25](aggregation.md#audit-2026-07-25--what-was-fixed-and-what-is-still-open):

- **~0.87 PJ of commercial non-electric, non-heat fuel is not soft-linked** (`Com.Other Energy` gas/oil/LPG/wood + gas and LPG cooking). Adding it needs a PyPSA-Eur `services cooking` / `total services` key, and those columns feed pypsa-eur's own logic — the substitution should be agreed before it is wired in.
- **Whether the remaining `total electricity services` end-uses should be split further.** Data centres are now exposed as a child (see log below); cooling, lighting, appliances and refrigeration are not, because PyPSA-Eur has no bus for them.
