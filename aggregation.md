...
| Date | Change | Reason |
|------|--------|--------|
| 2026-09-08 | **19 processes added to `mapping_processes.csv`** (second wave), found by the existing `test_every_local_vd_closes` sweep once the September-2026 `.vd` files (`scen_central_demande_haute_v01_260907_0709`, `..._v1_260828_2808`) landed in `data/` | New TIMES vintages the mapping had never seen: 10 building-heat devices (`RW{2F,4F,AP}OILN3`, `RW{N4F,NAP}PELN3`, `RH{N4F,NAP}HETN1`, `CHCSELCHP201`, `CHNCSELCHP301`, `CHNBPLTH101`) leaked exactly like the 2026-08-01 wave — up to **1.68 PJ of residential useful heat in 2025**, 0.07 PJ 2030, 1.00 PJ 2035, 0.11 PJ 2050, plus 0.004–0.08 PJ of tertiary. Service-output closure is exact again in every horizon of every local `.vd`. The other 9 (`ECHPP_AED_BGS_N`, `CHPINDGMX{IML,IPO}N00_N`, `ICHSTM{BIO,HFO,LFO}01`, `INMSTM{COK,HET}01`, `IOIPRCBIO01`) were invisible rather than leaking — `Industry` is read only by the `expect: zero` ammonia/methanol rules and `CHP` by no rule at all — so they gain a Sankey node but export nothing new. Labels reuse existing nodes throughout; every code is pinned in `UNMAPPED_2026_09_PROCESSES`. Indicator pages are unaffected: `model.py` backfills `sector`/`process_type` from `AllProcesses.csv`, and only `Aggregation Level 2` — which just the extraction rules read — was missing |
