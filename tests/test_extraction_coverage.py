...
def test_rule_count(rules):
    ...
    # + `services CHP heat` (2026-08-01, heat-leak fix: CHSADUM-DEM's useful heat
    #   was labelled `other demand` and matched by nothing).
    assert len(rules) == 59
...
def test_rule_metadata_columns_are_complete(mappings_dir):
    ...
    meta = load_rule_metadata(mappings_dir / "extraction_rules.csv")
    assert len(meta) == 59  # keep in sync with test_rule_count
